...
function calculate() {
  ...
  x1 = point1[0]
  y1 = point1[1]
  slope = Math.round((slope[0]/slope[1])*1000)/1000
  newYInt = Math.round((y1 - (newSlope * x1))*1000)/1000
  
  if (yInt>newYInt && slope>newSlope){
    MPx = (yInt - newYInt)/(slope - newSlope)
    if(y1<0 || x1<0){
      MPx = MPx*-1
    }
    console.log(MPx)
  ...
  else if (yInt>newYInt && slope<newSlope){
    MPx = (yInt - newYInt)/(newSlope - slope)
    console.log('2')

  }
  else if (yInt<newYInt && slope>newSlope){
    MPx = (newYInt - yInt)/(slope - newSlope)
    console.log('3')

  }
  else if (yInt<newYInt && slope<newSlope){
    MPx = (newYInt - yInt)/(newSlope - slope)
    console.log('4')
  }
 MPy = (slope * MPx) + yInt
